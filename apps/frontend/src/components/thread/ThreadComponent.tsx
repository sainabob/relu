...
import { useReluComputerStore, useSetIsSidePanelOpen } from '@/stores/relu-computer-store';
...
export function ThreadComponent({ projectId, threadId, compact = false, configuredAgentId, isShared = false, isNew = false, preCreatedThreadId }: ThreadComponentProps) {
  ...
  const openFileInComputer = useReluComputerStore((state) => state.openFileInComputer);
  const openFileBrowser = useReluComputerStore((state) => state.openFileBrowser);
  const setSandboxContext = useReluComputerStore((state) => state.setSandboxContext);
  ...
  // Sync Relu Computer sandbox context with current sandbox
  ...
  const handleModeStarterAction = useCallback(async (
    ...
    // Open the side panel to show ReluComputer
    if (!isSidePanelOpen) {
      toggleSidePanel();
    }
  ...
  const handleStarterPrompt = useCallback((prompt: string, placeholderInfo?: { start: number; end: number }) => {
    ...
    // Keep the side panel open to show ReluComputer
    if (!isSidePanelOpen) {
      toggleSidePanel();
    }
