...
export function useAuth() {
      ...
      try {
        ...
        const { data, error: signUpError } = await supabase.auth.signUp({
          ...
          options: {
            ...
            emailRedirectTo: 'relu://auth/callback',
          },
        });
   ...
   * Uses relu:// deep link - works when app is installed
   */
  const signInWithMagicLink = useCallback(async ({ email, acceptedTerms }: { email: string; acceptedTerms?: boolean }) => {
    try {
      ...
      const emailRedirectTo = `relu://auth/callback${params.toString() ? `?${params.toString()}` : ''}`;
      ...
      if (magicLinkError) {
        log.error('❌ Supabase rejected redirect URL:', {
          ...
          hint: 'Make sure relu://auth/callback is in Supabase Dashboard → Auth → Redirect URLs',
        });
      }
  ...
  const resetPassword = useCallback(async ({ email }: PasswordResetRequest) => {
    try {
      ...
      const { error: resetError } = await supabase.auth.resetPasswordForEmail(email, {
        redirectTo: 'relu://auth/reset-password',
      });
