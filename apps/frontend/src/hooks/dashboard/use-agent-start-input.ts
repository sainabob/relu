...
export function useAgentStartInput(options: UseAgentStartInputOptions = {}): UseAgentStartInputReturn {
  ...
  // For unauthenticated users, always show as Relu agent (for landing page modes panel)
  const isReluAgent = !user 
    ? true // Unauthenticated users always see Relu modes
    : isLoadingAgents 
