    ...
    (set, get) => ({
      ...
      setSandboxContext: (sandboxId: string | null) => {
        ...
        if (currentSandboxId !== sandboxId) {
          console.log('[ReluComputerStore] Sandbox context changed:', currentSandboxId, '->', sandboxId);
          set({
      ...
      clearFileState: () => {
        console.log('[ReluComputerStore] Clearing file state');
        set({
          ...initialFileState,
        });
      },
      ...
      reset: () => {
        console.log('[ReluComputerStore] Full reset');
        set(initialState);
      },
    }),
    {
      name: 'relu-computer-store',
    }
  )
);
...
// Sandbox context
export const useReluComputerSandboxId = () =>
  useReluComputerStore((state) => state.currentSandboxId);

export const useSetSandboxContext = () =>
  useReluComputerStore((state) => state.setSandboxContext);
