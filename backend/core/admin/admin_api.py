...
@router.post("/relu-agents/install-user/{account_id}")
async def admin_install_relu_for_user(
    account_id: str,
    replace_existing: bool = False,
    _: bool = Depends(verify_admin_api_key)
):
    """Install Relu agent for a specific user."""
    logger.debug(f"Admin installing Relu agent for user: {account_id}")
    
    service = ReluDefaultAgentService()
    agent_id = await service.install_relu_agent_for_user(account_id, replace_existing)
    
    if agent_id:
        return {
            "success": True,
            "message": f"Successfully installed Relu agent for user {account_id}",
            "agent_id": agent_id
        }
    else:
        raise HTTPException(
            status_code=500, 
            detail=f"Failed to install Relu agent for user {account_id}"
        )
